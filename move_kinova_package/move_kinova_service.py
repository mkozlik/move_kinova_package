...
class GrabServiceNode(Node):
    ...
    def grab_service_callback(self, request, response):
        ...
        try:
            ...
            self.transformed_pose = transformed_pose  # Store for later use
            ...
            self.picking_object = True
            self.send_place_above_goal(transformed_pose)  # Move above the object first
    ...
    def pouch_service_callback(self, request, response):
        try:
            ...
            self.send_pouch_action_goal()  # Implement this method to send a pouch command to the robot
        except Exception as e:
    ...
    def place_back_service_callback(self, request, response):
        try:
            ...
            self.send_place_above_goal(self.picked_pose)
    ...
    def send_action_goal(self, pose_stamped):
        ...
        pick_msg.target_pose.position.x += 0.12 # hacky way to not colide with the object
    ...
    def goal_response_callback(self, future):
        ...
        if not goal_handle.accepted:
            ...
            return
        self.accepted_object_id = self.latest_object_id  # Store the accepted object ID for later use
        self.get_logger().info(f"Goal accepted, object_id: {self.accepted_object_id}")
    ...
    def result_callback(self, future):
        ...
        request.picked_object = self.latest_detection  # Pass the entire detection message to the service
        if result.success:
            self.get_logger().info(f"Calling pick object service with object_id: {self.accepted_object_id}")
            #self.get_logger().info(f"latest detection: {self.latest_detection}")
            pick_object_future = self.pick_object_client.call_async(request)
            self.get_logger().info("Pick object service called, waiting for response...")
            time.sleep(4)
            self.get_logger().info(f"Pick object service response: {pick_object_future.result()}")
            self.send_place_above_after_goal(self.picked_pose)  # Move above the object first
        else:
    ...
    def send_sniff_action_goal(self):
        ...
        sniff_msg.target_pose.orientation.x = 0.707
        sniff_msg.target_pose.orientation.y = 0.0
        sniff_msg.target_pose.orientation.z = 0.707
        sniff_msg.target_pose.orientation.w = 0.0
        sniff_msg.move_gripper = 0
        sniff_msg.object_attached = True
    ...
    def sniff_result_react(self, future):
        ...
        threat_request.sensor_value = sensor_reading  # Example threat level
        time.sleep(2)  # Ensure the sensor reading is updated before sending the request
        self.set_threat_client.call_async(threat_request)
        self.get_logger().info(f"Sensor reading at sniff: {sensor_reading}")
    ...
    def return_home_action_goal(self):
        ...
        home_msg.target_pose.position.x = -0.25
        home_msg.target_pose.position.y = 0.0
        home_msg.target_pose.position.z = 0.35
        home_msg.target_pose.orientation.x = 0.0
        home_msg.target_pose.orientation.y = -0.77
        home_msg.target_pose.orientation.z = 0.0
        home_msg.target_pose.orientation.w = 0.607
        home_msg.move_gripper = 0
        home_msg.object_attached = True
    ...
    def send_pouch_action_goal(self):
        pouch_msg = SimpleMove.Goal()
        pouch_msg.target_pose.position.x = 0.0
        pouch_msg.target_pose.position.y = -0.2
        pouch_msg.target_pose.position.z = 0.15
        pouch_msg.target_pose.orientation.x = 0.64
        pouch_msg.target_pose.orientation.y = -0.3
        pouch_msg.target_pose.orientation.z = 0.64
        pouch_msg.target_pose.orientation.w = 0.3
        pouch_msg.move_gripper = 2  # Custom flag to indicate pouch
        pouch_msg.object_attached = True

        send_pouch_future = self.action_client.send_goal_async(
            pouch_msg,
            feedback_callback=self.pouch_feedback_callback
        )

        send_pouch_future.add_done_callback(self.pouch_response_callback)

    def pouch_response_callback(self, future):
        goal_handle = future.result()

        if not goal_handle.accepted:
            self.get_logger().info("Pouch goal rejected")
            return

        self.get_logger().info("Pouch goal accepted")

        result_future = goal_handle.get_result_async()
        result_future.add_done_callback(self.pouch_result_callback)

    def pouch_result_callback(self, future):
        result = future.result().result
        self.get_logger().info(f"Pouch action completed, success: {result.success}")
        time.sleep(2)  # Wait for any final movements to complete
        pouch_request = SetBool.Request()
        pouch_request.data = True
        self.pouch_object_client.call_async(pouch_request)  # Notify that release is done
        self.return_home_action_goal()

    def pouch_feedback_callback(self, feedback_msg):
        feedback = feedback_msg.feedback
        self.get_logger().info(f"Status: {feedback.status}")
    ...
    def send_place_back_action_goal(self, pose_stamped):
        ...
        pick_msg.object_attached = True

        send_goal_future = self.action_client.send_goal_async(
            pick_msg,
            feedback_callback=self.place_back_feedback_callback
        )
    ...
    def place_back_result_callback(self, future):
        ...
        time.sleep(2)  # Wait for any final movements to complete
        place_back_request = SetBool.Request()
        place_back_request.data = True
        self.place_back_client.call_async(place_back_request)  # Notify that place
        self.send_place_above_after_goal(self.picked_pose)  # Move back to home position after placing back

    def place_back_feedback_callback(self, feedback_msg):
        feedback = feedback_msg.feedback
        self.get_logger().info(f"Status: {feedback.status}")

    #-----------------place above object action----------------

    def send_place_above_goal(self, pose_stamped):
        ...
        pick_msg.target_pose.position.x += 0.20 # Move before the object
        ...
        send_goal_future.add_done_callback(self.goal_place_above_response_callback)
        pick_msg.target_pose.position.x -= 0.20 # Move before the object
        pick_msg.target_pose.position.z -= 0.1 # Move above the object

    def goal_place_above_response_callback(self, future):
        goal_handle = future.result()

        if not goal_handle.accepted:
            self.get_logger().info("Place above goal rejected")
            return

        self.get_logger().info("Place above goal accepted")

        result_future = goal_handle.get_result_async()
        result_future.add_done_callback(self.place_above_result_callback)
    
    def place_above_result_callback(self, future):
        result = future.result().result
        self.get_logger().info(f"Place above completed, success: {result.success}")
        if result.success:
            time.sleep(1)  # Wait for the robot to move above the object
            if self.picking_object:
                self.send_action_goal(self.transformed_pose)  # Then send the actual grab goal
            else:
                self.send_place_back_action_goal(self.picked_pose)  # Then send the actual place back goal

     #-----------------place above object action after----------------

    def send_place_above_after_goal(self, pose_stamped):
        ...
        send_goal_future.add_done_callback(self.goal_place_above_after_response_callback)
        pick_msg.target_pose.position.x -= 0.20 # Move before the object
        pick_msg.target_pose.position.z -= 0.1 # Move above the object

    def goal_place_above_after_response_callback(self, future):
        goal_handle = future.result()
        ...
        result_future = goal_handle.get_result_async()
        result_future.add_done_callback(self.place_above_result_after_callback)
    
    def place_above_result_after_callback(self, future):
        result = future.result().result
        self.get_logger().info(f"Place above completed, success: {result.success}")
        if result.success and self.picking_object:
            time.sleep(1)  # Wait for the robot to move above the object
            self.send_sniff_action_goal()  # Then send the actual grab goal

        elif result.success and not self.picking_object:
